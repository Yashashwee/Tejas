...
public class ObjParser 
{
	...
	*/
	public static void buildStaticInstructionTable(String executableFile) 
	{
		...
		System.out.println("OBJ parser 127 number of line" + Long.toString(noOfLines));
		if(noOfLines==0) {
...
//		}
			...
			EmulatorPacketList arrayListPacket, GenericCircularQueue<Instruction> inputToPipeline)
	{
		...
		if(EmulatorConfig.emulatorType==EmulatorType.none) {
			// System.out.println("Objparser 616");
		...
		Instruction staticMicroOp, dynamicMicroOp, tempMicroOp;
		...
		while(true)
		{
			...
			// if(dynamicMicroOp.getOperationType() == OperationType.load)
			// {
			// 	//Create an mfence instruction to add with the load instruction
			// 	tempMicroOp = getDynamicMicroOp(staticMicroOp);
			// 	tempMicroOp.setOperationType(OperationType.mfence);
			// 	++microOpIndex;
			...
			// 		inputToPipeline.enqueue(tempMicroOp); //append mfence microp
			// 	}

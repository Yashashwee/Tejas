...
import main.Main;
import memorysystem.MemoryModels;
...
public class XMLParser 
{
	...
	{ 
		try 
		{
			...
			if(SimulationConfig.enableCBD){
				System.out.println("CBD Enabled ... Setting up constraints");
				cbd.init_cbd();
				setConstraints();
				System.out.println("\n------------");
				System.out.println(cbd.getConstraintsSize()+" constraints registered. Execution Time Delta : "+SimulationConfig.CBD_TimeDelta );
				System.out.println("\n------------");
			}
		} 
		catch (Exception e) 
	...
	private static void setConstraints() throws Exception {

		
		NodeList nodeLst = doc.getElementsByTagName("Constraints");
		Node constraintsNode = nodeLst.item(0);
		Element constraintsElements = (Element) constraintsNode;

		NodeList Constraints = constraintsElements.getElementsByTagName("Constraint");

		for(int i=0;i<Constraints.getLength();i++) {
			Element constraint = (Element)Constraints.item(i);
			
			String class_name  = getImmediateString("Class",constraint);
			String method_name = getImmediateString("Method",constraint);
			...
			if(class_name != null && method_name != null) {
				...
				Node argumentNode = constraint.getElementsByTagName("Arguments").item(0);
				
				if(argumentNode!= null) {

					NodeList arguments = argumentNode.getChildNodes();

					if(arguments.getLength() != 0) {

						
						for(int j=0;j<arguments.getLength();j++) {
							Node argumentnode = (Node)(arguments.item(j));
							...
							Object arg=null;
							if(argument.getTagName() == "Double") {
								arg = new Double(Double.parseDouble(argumentnode.getTextContent()));
							}
						
							else if(argument.getTagName() == "Integer") {
								arg = new Integer(Integer.parseInt(argumentnode.getTextContent()));
							}

							else if(argument.getTagName() == "String") {
								arg = argumentnode.getNodeValue();
							}

							else if(argument.getTagName() == "Boolean") {
								arg = new Boolean(Boolean.parseBoolean(argumentnode.getTextContent()));
							}

							else {
								System.out.println("Unkown argument type "+argument.getTagName()+". Ignoring Argument..");
							
							}

							if(arg != null) {
								if(c.arguments == null) {
									c.arguments = new Vector<Object>();
								}
								c.arguments.add(arg);
							}
				...
				cbd.setup(c.class_name,c.method_name,c.arguments);
			} else {
				System.out.println("Class name was "+class_name+". Method name was "+method_name);
			}
	...
	static ISAType getISAType(String isaType) {
                ISAType t = null;

                try {
                        t = ISAType.valueOf(isaType);
                } catch (Exception e) {
                        misc.Error.showErrorAndExit("Error in setting the isa type argument." +
                                        "\nExpected values : x86, arm or riscv");
                }
	...
	private static void setISAType()
	{
		NodeList nodeLst = doc.getElementsByTagName("ISA");
		if(nodeLst.getLength()==0)
		{
			ISAConfig.isatype=ISAType.x86;
		}
		else
		{			
            Node isaNode = nodeLst.item(0);
            Element isaElmnt = (Element) isaNode;

            ISAConfig.isatype = getISAType(getImmediateString("ISAType", isaElmnt));
        }
	}
	...
	private static void setSimulationParameters()
	{
		...
		SimulationConfig.enableCBD = Boolean.parseBoolean(getImmediateString("EnableCBD",simulationElmnt));
		SimulationConfig.CBD_TimeDelta = Long.parseLong(getImmediateString("CBDTimeDelta",simulationElmnt));
		...
		if(getImmediateString("MemoryModel", simulationElmnt).compareTo("SC")==0)
		{
			SimulationConfig.memoryModel = MemoryModels.SC;
		}else if(getImmediateString("MemoryModel", simulationElmnt).compareTo("SC")==0)
		{
			SimulationConfig.memoryModel = MemoryModels.TSO;
		}else 
		{
			SimulationConfig.memoryModel = MemoryModels.WEAK;
		}

		SimulationConfig.instnsWarmup=Long.parseLong(getImmediateString("NumInsForCacheWarmup",simulationElmnt));

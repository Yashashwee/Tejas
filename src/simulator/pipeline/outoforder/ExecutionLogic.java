...
public class ExecutionLogic extends SimulationElement {
	...
	public void handleExecutionCompletion(ReorderBufferEntry reorderBufferEntry)
	{
		...
		if(reorderBufferEntry.getInstruction().getOperationType()==OperationType.syscall)
		{
			execEngine.getCoreMemorySystem().getiTLB().flushTlb();
			execEngine.getCoreMemorySystem().getdTLB().flushTlb();
			//System.out.println("flushed tlb");
		}
		...
		if(reorderBufferEntry.getInstruction().getDestinationOperand() != null
				...
				|| reorderBufferEntry.getInstruction().getOperationType()==OperationType.clflush)
		{
			if(reorderBufferEntry.getInstruction().getOperationType()==OperationType.clflush)
			{
				
				// Instruction ins1 = reorderBufferEntry.getInstruction();
				
				// System.out.println(ins1.toString());
				// //Long addr = ins1.getSourceOperand1MemValue();
				// //int op1 = reorderBufferEntry.getOperand1PhyReg1();
				// //System.out.println("Exec unit "+ addr);
				// //System.out.println(reorderBufferEntry.getOperand1PhyReg1());
	...
	{
	  numResultsBroadCastBusAccess = 0;
	}
